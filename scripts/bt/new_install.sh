...
Install_Bt(){
	...
	wget -O panel.zip ${miui_Url}/install/src/panel6.zip -T 10
	...
	wget -O /www/server/panel/install/public.sh ${miui_Url}/install/public.sh -T 10
    ...
    chattr +i /www/server/panel/install/public.sh
	wget -O /www/server/panel/install/check.sh ${miui_Url}/install/check.sh -T 10
	chattr +i /www/server/panel/install/check.sh
	rm -f panel.zip
...
+----------------------------------------------------------------------
| The WebPanel URL will be http://SERVER_IP:8484 when installed.
+----------------------------------------------------------------------
"
while [ "$go" != 'y' ] && [ "$go" != 'n' ]
do
...
echo -e "\033[33mrelease the following port (8484|888|80|443|20|21) in the security group\033[0m"
echo -e "=================================================================="

endTime=`date +%s`
((outTime=($endTime-$startTime)/60))
echo -e "Time consumed:\033[32m $outTime \033[0mMinute!"
rm -rf new_install.sh
